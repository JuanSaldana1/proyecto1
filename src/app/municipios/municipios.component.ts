...
export class MunicipiosComponent implements OnInit {
    ...
    getProvincias(event: any) {
        ...
        return this.municipiosApi.getProvinciasByComunidad(this.codigoComunidad).subscribe(response => {
            this.provinciasList = response;
            for (let provincia of this.provinciasList.data) {
                console.log(provincia);
                this.municipiosApi.getMunicipiosByProvincia(provincia.CPRO).subscribe(response => {
                    this.municipiosList = response;
                    for (let municipio of this.municipiosList.data) {
                        console.log(municipio);
                        this.municipios.push({value: municipio.CMUM, text: municipio.DMUN50});
                    }
                });
            }
        });
    ...
    getMunicipios(event: Event) {
        return this.municipiosApi.getMunicipiosByProvincia(this.provincia).subscribe(response => {
            this.municipiosList = response;
